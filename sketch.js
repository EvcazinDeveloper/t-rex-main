...
function setup() {
  ...
  restart = createSprite(width/600*300, height/280*150);
  restart.addImage(restartimg);
  restart.scale = 0.5;
  restart.visible = false;

  //criar um sprite do trex
  trex = createSprite(width/600*50, height/205*160, width/600*20, height/200*50);
...
function draw() {
  ...
  if ((touches.length > 0 || keyDown("space")) && trex.y >= 840) {
    trex.velocityY = -10;
    somSalto.play();
    touches = [];
  }
...
function geraObstaculos() {
  if (frameCount % 60 === 0) {
    var obstaculo = createSprite(width/600*601, height/205*195, width/600*10, height/200*40);
